...
export default function App() {
  return (
    <div className={`mx-auto my-8 flex max-w-[984px] flex-wrap justify-center gap-6 bg-gray-200 font-mono text-[#00386b]`}>
      <aside
        className={`flex flex-col gap-6 rounded-md border border-black bg-gray-100 shadow-lg md:w-[640px] md:flex-row md:gap-0 lg:w-[320px] lg:flex-col lg:gap-6`}
      >
        ...
        <ul className={`flex flex-col gap-2 rounded-b-md px-6 pb-6 text-base font-medium md:pt-6 lg:pt-0`}>
          {links().map((link) => (
            <li key={link.text} className={`flex flex-row items-center gap-6`}>
              {link.icon}
      ...
      <section className={`hidden w-[640px] justify-between rounded-md border border-black bg-gray-100 p-6 shadow-lg md:flex md:flex-col`}>
        <div className={`flex flex-col gap-8`}>
          <h2 className={`flex flex-row gap-2 border-b border-black pb-2 text-xl font-bold`}>
            Hi there
            <div className={`animate-wiggle-more animate-duration-[600ms] animate-infinite animate-ease-out`}>👋</div>, I am Vincent Huang
          ...
          <ul className={`rounded-md bg-gray-200 text-base font-medium`}>
            {introduction().map((description) => (
              <li key={description} className={`flex flex-row items-center gap-4`}>
                <span>◾️</span>
                {description}
              </li>
            ))}
          ...
          <div className={`flex flex-row items-center border-b border-black pb-2`}>
            ...
            <a href={`https://github.com/Jian-Min-Huang`} target={`_blank`} rel="noreferrer">
              ⋯⋯ more
            </a>
          </div>
          <div className={`flex flex-wrap gap-4 text-sm`}>
            <a href={`https://github.com/b2etw/Spring-Kotlin-iThome-2021`} target={`_blank`} rel="noreferrer">
            ...
            <a href={`https://github.com/PureFuncInc/pure-backend-practice`} target={`_blank`} rel="noreferrer">
              <div className={`flex w-[287px] flex-col gap-4 rounded-md border border-black p-2`}>
                <span className={`flex flex-row items-center gap-2 font-semibold`}>
                  <FaFloppyDisk size={16} />
            ...
            <a href={`https://github.com/taiwan-kotlin-user-group/emoji-jvm-string`} target={`_blank`} rel="noreferrer">
              <div className={`flex w-[287px] flex-col gap-4 rounded-md border border-black p-2`}>
                <span className={`flex flex-row items-center gap-2 font-semibold`}>
                  <FaFloppyDisk size={16} />
                  emoji-jvm-string
            ...
            <a href={`https://github.com/PureFuncInc/spring-boot-3-practice`} target={`_blank`} rel="noreferrer">
              <div className={`flex w-[287px] flex-col gap-4 rounded-md border border-black p-2`}>
                <span className={`flex flex-row items-center gap-2 font-semibold`}>
                  <FaFloppyDisk size={16} />
